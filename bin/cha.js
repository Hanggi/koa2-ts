...
function getPrice(coin, id_index) {
	...
	request("https://api.coinone.co.kr/trades?currency=" + coin, (err, res, body) => {
		...
		if (!coinArr[id_index]) {
			...
			coinArr[id_index] = money / data[data.length - 1].price;

			coinPriceArr[id_index].qty = coinArr[id_index]

			coinPriceArr[id_index].p = coinArr[id_index] * coinPriceArr[id_index].price;

			coinPriceArr[id_index].max = data[data.length - 1].price;

			coinPriceArr[id_index].min = data[data.length - 1].price;
		...
		coinPriceArr[id_index].min = Math.min(data[data.length - 1].price, coinPriceArr[id_index].min)

		coinPriceArr[id_index].puls = (coinPriceArr[id_index].max - coinPriceArr[id_index].min) * coinArr[id_index]
		...
		coinPriceArr[id_index].p = Math.round(coinArr[id_index] * coinPriceArr[id_index].price);
				...
				for (let i = 0; i < coinPriceArr.length; i++) {
					...
					for (let b = 0; b < coinPriceArr.length; b++) {
						...
						if (tmpLast && Number(tmpLast[i][b]) < Number(Arr[i][b])) {
							...
							// consolesß.log("111")
							str += chalk.greenBright(Arr[i][b]) + " | ";

						} else if (tmpLast && Number(tmpLast[i][b]) > Number(Arr[i][b])) {

							str += chalk.red(Arr[i][b]) + " | ";

						} else {

							str += chalk.white(Arr[i][b]) + " | ";
