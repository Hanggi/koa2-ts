...
function getPrice(coin, id_index) {
    request("https://api.coinone.co.kr/trades?currency=" + coin, (err, res, body) => {
        ...
        qty = Math.round(qty)
        console.log("[" + coin.toUpperCase() + "]前一分钟交易量：" + qty + "，价格" + (data[data.length - 1].price > data[data.length - 500].price ? "上涨": "下跌") + "到：" + data[data.length - 1].price + "，交易额达到：" + qty*data[data.length - 1].price/100000000 + "亿韩元")
        // console.log(heat)
